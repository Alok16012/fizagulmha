...
export default function BlogShare({ title, slug }: BlogShareProps) {
  ...
  const url = `https://clatians.com/blogs/${encodeURIComponent(slug)}`;
  const waText = encodeURIComponent(`${title}\n\nRead more: ${url}`);
  const twitterText = encodeURIComponent(`${title} via @CLATians`);
    ...
    <div className="mt-6 pt-6 border-t border-gray-100">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          style={
            copied
              ? { background: '#fff1e8', color: '#f77420', borderColor: '#f77420' }
