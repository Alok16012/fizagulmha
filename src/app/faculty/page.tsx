          ...
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="max-w-2xl">
              <span className="text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full"
                style={{ background: 'rgba(8,189,128,0.16)', color: '#5DE2B2', border: '1px solid rgba(93,226,178,0.24)' }}>
                Our Faculty
              </span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
            {faculty.map((f) => {
              ...
              return (
                ...
                >
                  <div className="p-5" style={{ background: f.bg || '#E6FAF4' }}>
                    {f.photo ? (
                      <img
                        src={f.photo}
                        alt={f.name}
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4">
                      {f.tags.slice(0, 3).map((tag) => (
                        <span key={tag} className="text-[10px] font-bold px-2 py-1 rounded-full"
                          style={{ background: '#F0FDF9', color: accent, border: '1px solid #D1FAE5' }}>
                          {tag}
                        </span>
                      ))}
                    </div>
